...
6. サイトは4分野構成です。`index.html`（観測室）はヒーローと「4つの観測窓」を持つハブで、`signals.html`（最新情報 — 政策シグナルのフィルタ付きコンソール）、`people.html`（人 — 博士の曲線・キャリアの構造・研究者の移動・博士の経済の4章）、`money.html`（お金 — 資金の流れ・大学と研究機関の財務・学術出版・研究助成機関の構造の11章）、`papers.html`（研究 — 40年の競争・分野の地形・テーマの変遷）に分かれます。共有ヘルパーは `obs-core.js`、ページ別の描画は `hub.js` / `signals.js` / `people.js` / `papers.js` / `money.js`。各ページのフッターの出典台帳がそのページのデータソースの原典と接続状態を示します（ハブは全分野分）。Canvas粒子・D3・GSAP ScrollTriggerを使用します。`analytics.html` は旧URL向けのリダイレクトです。
